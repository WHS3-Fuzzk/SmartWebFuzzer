...
import contextlib, psycopg2.extras as _E
...
class DBReader:
    ...
    def _select_request(self, prefix: str, rid: int) -> Dict[str, Any]:
        with self._cur() as c:
            ...
            c.execute(
                f"SELECT * FROM {body_tbl} WHERE {prefix}request_id=%s", (rid,)
            )
            body = c.fetchone()

        return {"meta": meta, "headers": headers, "query_params": params, "body": body}

    def _select_response(self, prefix: str, rid: int) -> List[Dict[str, Any]]:
        with self._cur() as c:
            c.execute(
            ...
            headers = c.fetchall()
            c.execute(
                f"SELECT * FROM {prefix}response_body "
                f"WHERE {prefix}response_id=%s",
                (resp_id,),
            )
            body = c.fetchone()
